...
def start(core: VACore):
    manifest = {
        ...
        "options_label": {
            ...
            "ttsEngineId2": "ID дополнительного движка озвучки. Всегда озвучивает результат на той машине, где запущена Ирина (без веб-интерфейса)",
            # двиг для прямой озвучки на сервере. Если пуст - используется ttsEngineId
            "playWavEngineId": "ID движка воспроизведения аудио. Если есть проблемы - попробуйте сменить на audioplayer или sounddevice",
            "linguaFrancaLang": "Язык для библиотеки lingua-franca конвертирования чисел",
            # язык для библиотеки lingua-franca конвертирования чисел
            ...
            "logPolicy": "all|cmd|none . Когда распознается речь с микрофона - выводить в консоль всегда | только, если является командой | никогда",
            # all | cmd | none
        ...
        "default_options": {
            ...
            "GPT": {
                ...
                "gpt_command_prompt": ("Говори от имени голосового ассистента."
                                       "Тебе нужно выбрать строку исходя из списка строк, названий команд: {commands}"
                                       "\nВ ответ можно написать только строку из списка, "
                                       "которой соответствует команда и обязательно строго без изменений.")
            }
...
def start_with_options(core: VACore, manifest: dict):
    ...
    core.openai_api_key = options["GPT"]["openai_api_key"]
    core.openai_api_base = options["GPT"]["openai_api_base"]
    core.gpt_command_prompt = options["GPT"]["gpt_command_prompt"]
