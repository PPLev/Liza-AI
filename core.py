...
class Core(JaaCore, metaclass=MetaSingleton):
    ...
    async def run_input(self, input_str):
        await self.input.event(self, input_str)
...
@core.input.register
async def m2(core: Core, input_str):
    print("ok")


async def main():
    await core.run_input("sdsdsd")
